...
#'
#' This function defines the server logic required to draw a histogram in the app.
#' It also defines multiple global variables and functions used in the app.
#'
...
server <- function(input, output, session) {
  ...
  #' Clear ingredient dataframe
  #'
  #' This function clears the ingredient dataframe used in the app.
  #'
  #' @return The cleared ingredient dataframe
  #' 
  #' @export
  ...
  #' Clear all data
  #'
  #' This function clears all data used in the app, including the ingredient dataframe and edit data.
  #'
  #' @return NULL
  #' 
  #' @export
  ...
  #' Clear edit data
  #'
  #' This function clears the edit data used in the app, including the meal ID, meal name, and serving size.
  #'
  #' @return NULL
  #' @export
  ...
  #'
  #' This function loads ingredient data into the app from a specific object.
  #'
  #' @param my_object The ingredient data object
  #' @return The updated ingredient dataframe
  #' 
  #' @export
  load_ingredients_data <- function(my_object) {
    ...
    for (i in 1:nrow(my_object)) {
      ...
      #' Get the actual working ingredient dataframe for dplyr
      #'
      #' This function takes no arguments, and returns the working ingredient
      #' dataframe for dplyr, which is used to calculate nutrient information for a recipe.
      #'
      #' @return A dataframe showing the actual working ingredient dataframe for dplyr.
      #'
      #' @examples
      #' input_measure <- get_actual_ingredient_df()
      #' print(input_measure)
      #' 
      #' @export
  ...
  #'
  #' This function creates a reactive variable that stores a list of ingredients.
  #' It takes no arguments and returns a reactiveVal object.
  #'
  #' @return A reactiveVal object that stores a list of ingredients.
  #'
  #' @examples
  ...
  #' Delete recipe
  #'
  #' This function deletes a recipe from the database, and refreshes the recipe table.
  #' It takes no arguments, but relies on the \code{g_edit_meal_id} reactive variable and the \code{input$delete_recipe} input values.
  #'
  #' @examples
  ...
  #' Delete a recipe when user clicks the button
  #'
  #' This function deletes a recipe from the database and refreshes the table, when the user clicks on the "Delete" button in the app. 
  #' It depends on the \code{g_edit_meal_id} reactive variable, the \code{input$delete_recipe} input value, and the \code{db} module.
  #'
  ...
  #'
  #' This function saves the current log, including user email, meal name, serving size, and recipe data, to the log table in the database, when the user clicks on the "Save Log" button in the application. 
  #' It depends on the \code{ingredients_list} value, the \code{input$meal_name} and \code{input$serving} values, and the \code{db} module.
  #'
  #' @importFrom shiny observeEvent
  ...
  #'
  #' This function handles the how the "Delete Entry" button in the Interface shows based on whether a row is selected in the table. 
  #' When a row is selected, the "Delete Entry" button is shown, and when no rows are selected, the button is hidden. 
  #' It depends on the \code{input$log_table_rows_selected} input value and the \code{show} and \code{hide} functions from the shiny package.
  #'
  ...
  #'
  #' This function handles the "Delete Entry" button click in the Interface by deleting the selected entry from the database. 
  #' It depends on the \code{input$log_table_rows_selected} and \code{click} values and the \code{import} and \code{delete_log} functions from "db" package.
  #'
  #' @importFrom shiny observeEvent
  #' @import db import click delete_log
  #' 
  #' @examples
  #' observeEvent(input$delete_entry, {
  #'   delete_selected_log_entry()
  #' })
  #' @export
  #handle button click
  observeEvent(input$delete_entry, {
  ...
  #'
  #' This function loads the user log data from the database and displays it in a table. 
  #' It depends on the \code{input$load_log}, \code{output$log_table}, \code{import}, \code{get_log}, and \code{renderDT} functions from the "db" and "DT" packages.
  #'
  #' @importFrom shiny observeEvent
  #' @importFrom shiny renderDT
  #' @import db import get_log
  #' @import DT datatable
  #'
  #' @examples
  ...
  #' Save Recipe
  #'
  #' When the user clicks on "Save Recipe" button, this function opens a modal dialog for typing the meal name.
  #' If the user submits the meal name, the function checks if the user is logged in and if the meal name is valid.
  #' If everything is valid, it converts the list of ingredients to a JSON text and saves it to the database as a recipe.
  #'
  #' @keywords save recipe, modal dialog, JSON text, database
  #' @importFrom shiny observeEvent showModal modalDialog textInput footer modalButton actionButton toJSON
  #' @export
  observeEvent(i
  # Save recipe
  ...
  #' Submit Save Recipe
  #'
  #' The function is called when the user clicks the "Submit" button in the Save Recipe modal.
  #' It checks if the user is logged in, the meal name is valid and at least one ingredient is added to the list.
  #' If everything is valid, it converts the list of ingredients to a JSON text and saves it to the database as a recipe.
  #' Or it displays an error message.
  #'
  #' @keywords submit save recipe, JSON text, database, modal dialog
  #' @importFrom shiny observeEvent showModal modalDialog easyClose delay updateTextInput click
  #' @importFrom jsonlite toJSON
  #' @export
  ...
  #' Login with Google
  #'
  #' This function authenticates a user using Google credentials.
  #'
  #' @param input$g.email The email address associated with the Google account.
  #' @param input$g.name The name associated with the Google account.
  #' @keywords Google authentication
  #' @export
  ...
  #'
  #' This function creates a JSON file of the current list of ingredients and downloads it.
  #'
  #' @keywords downloadHandler
  #' @export
  #' @return JSON file
  # Download Ingredients as json file
  ...
  #' Calculate the nutrition dataframe for the recipe
  #'
  #' This function calculates the nutrition data for the recipe based on the ingredient and measurement dataframe. 
  #' Creates a dataframe with columns for nutrient name, value, ID, unit, quantity, and food ID.
  #'
  #' @return A reactive data frame with the calculated nutrition data.
  #'
  #' @keywords reactive data frame nutrition
  #' 
  #' @export
